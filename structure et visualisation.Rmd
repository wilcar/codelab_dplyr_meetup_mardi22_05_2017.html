...
## Identification des données manquantes colonne par colonne
...
Effectifs et fréquences des valeurs manquantes
...
## Données manquantes en fonction des années

## Identification des données manquantes en fonction des années
```{r}

library(dplyr)

# Frequences des données NA en pourcentage de chaque colonne
missing_years <- voyages %>%
                  select (1:87) %>%
                  group_by (yearam) %>% 
                  summarise_each(funs(round(100*mean(is.na(.)) ,1)))

# Effectifs  des données NA en pourcentage de chaque colonne
missing_years2 <- voyages %>%
                  select (1:87) %>%
                  group_by (yearam) %>% 
...
##Identification des données manquantes en fonction des années: exemple de natinimp

### Préparation des données
```{r}

# proposition 1
missing_years_natinimp <- voyages %>%
                  count (yearam, natinimp =complete.cases(natinimp))
                   

missing_years_natinimp$natinimp <- factor(missing_years_natinimp$natinimp, labels=c("non-identifié", "identifié"))

# proposition 2

missing_years_natinimp2 <- voyages %>%
                  count (yearam, pavillon = complete.cases(natinimp)) %>%
                  mutate(pavillon = c("non-identifié","identifié")[pavillon + 1])



```

http://stackoverflow.com/questions/30943167/how-can-i-replace-true-and-false-values-with-1-and-0-when-exporting-data-in-r

#### Diagrammes en bâtons empilés (stacked)
 
```{r eval = FALSE}

library(ggplot2)

flags_na_natinimp_stacked  <- ggplot (missing_years_natinimp2, aes(x=yearam, y=n, width = 0.5))  
flags_na_natinimp_stacked +
  geom_bar(stat="identity", aes(fill = (pavillon))) +
  scale_x_continuous(breaks=seq(1500, 1860, by=25)) +
  theme_bw() +
  ggtitle ("Part des pavillons de navires négriers inconnus (caractère natinimp)") 
