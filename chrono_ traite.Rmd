...
variable yearam: Year of arrival at port of disembarkation (imputed) (spss codebook)  
http://www.slavevoyages.org/voyage/understanding-db/methodology-7

variable slaximp: Imputed total slaves embarked (spss codebook)  
http://www.slavevoyages.org/voyage/understanding-db/methodology-15

```{r eval = FALSE}
# données manquantes

sum(is.na(voyages$yearam)) 
sum(is.na(voyages$slaximp)) 
```
...
range # Affichage dans la console


complete_n_voyages <- left_join(range, n_voyages)

complete_n_voyages # Affichage dans la console
...
##  Nombre moyen des expéditions de traite par année 
...
# Exploration de la chronologie de la traite à l'aide: représentations graphiques

## Représentations graphiques élementaires par l'intermédiaire de R-base
...
## Représentations graphiques avancées par l'intermédiaire du package ggplot2
...
### Calcul et ajout d'une moyenne mobile
...
gvis_chrono2 <- gvisColumnChart(complete_n_voyages_rename,
options=list(
                              legend="none",
                              lineWidth=2, pointSize=0,
                              title="Trans-Atlantic slave trade voyages (yearam variable)",
...
Lorsque presque tous les individus étudiés sont caractérisés par des valeurs différentes, on peut procéder à leur regroupement dans des intervales de valeurs appelés *classes* afin de faciliter la lecture, l'analyse et interprétation des données.  

Le découpage en classes d'une population est une opération appelée *discrétisation*.
Cette transformation engendre une perte d'information et par conséquent, le processus de partition en classes revêt une grande importance. (Sède-Marceau, 2010; Saly-Giocanty, 2005, p.31)

* Une classe est un intervalle de valeurs ue peut prendre le le caractère quantitatif étudié.  
* L'amplitude d'une classe est la longueur de l'intervalle de valeurs. Les classes peuvent être d'amplitudes différentes.  


Les classes ne se chevauchent jamais. La borne supérieure est généralement exclue de l'intervalle de classe. On dit alors que les classes s'expriment: "bornes exclues à droite" (Py, 2013; p. 30). 


* L'intervalle noté: [a;b[ est noté dans R [a,b) est exclu à droite  
* L'intervalle noté: ]a;b] est noté dans R (a,b] est exclu à gauche


### R-Base

#### Discrétisation empirique

```{r  eval=FALSE}

# bornes exlues à gauche

cut_chrono <- cut(voyages$yearam,
                   breaks=c(1500 ,1600, 1700 ,1800 ,1900),
                   dig.lab = 4)      # Suppression de la notation scientifique 
...
cut_chrono <- as.data.frame(cut_chrono1)

```

#### Discrétisation selon des intervalles égaux

```{r}

# Dermination manuel du nombre de classes 

cut_chrono <- cut(voyages$yearam,
                  breaks = 4,
                  dig.lab = 4)   
          
# Détermination des bornes min / max et de l'amplitude des classes

cut_chrono <- cut(nantes1$voy1imp,
...
# Expéditions
cut_chrono <- voyages %>%
               select (yearam, slaximp, shipname) %>%
               mutate (periodes = cut(yearam, breaks = seq(1500,1600, by =50),
               include.lowest = TRUE,
               right = FALSE,  
               dig.lab = 4))%>%
               count (periodes)
...
# esclaves embarqués
cut_chrono <- voyages %>%
               select (yearam, slaximp) %>%
               mutate (periodes = cut(yearam, breaks = seq(1500,1900, by =50),
               include.lowest = TRUE,
               right = FALSE,
               dig.lab = 4)) %>%
               filter (complete.cases(slaximp))%>%
