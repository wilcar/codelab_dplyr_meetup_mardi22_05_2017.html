...
Un tableau de données ou *data.frame* constitue une structure de données fondamentale de **R**. Il se compose d'une collection d'objets de différents types (*numeric, factor, character*) de longueur similaires accessibles par leur indice de position (R: A Language and Environment for Statistical Computing - Reference index, p 159).
...
La fonction *colnames* est utilisée pour obtenir un affichage séquentiel des noms de colonnes. Chaque colonne est caractérisée par un *indice* de position au sein du tableau. l'indice du premier élément de chaque ligne est signalé dans la marge gauche de la console.
...
Un *factor* constitue un objet particulier de R employé pour stocker les modalités d'un caractère qualitatif. 
...
Objets particuliers de R pour représenter les variables numeriques. Un caractères est dit quantitatif si ses modalités sont mesurables. Le caractère quantitatif prend alors le nom de *variable statistique* et ses différentes modalités sont les *valeurs* possibles de la variable (Py, 1996).
