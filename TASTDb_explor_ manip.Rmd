...
Un tableau de données ou *data.frame* constitue une structure de données fondamentale de *R*. Il est composé d'une collection d'objets de différents types (numeric, factor, character.) de longueur similaires accessibles par le indice de position (R: A Language and Environment for Statistical Computing - Reference index, p 159).

# Composition du tableau de données 

## Dimensions

Un tableau de données est un tableau à deux dimensions. La fonction *dim(){base}*  permet d'en connaître le nombre de lignes et de colonnes (Becker, R. A., Chambers, J. M. and Wilks, A. R. (1988) The New S Language. Wadsworth & Brooks/Cole.).
...
### La fonction wich(){base}
La fonction *wich* est une fonction primitive de R qui permet d'identifier  l'indice de position d'un vecteur (d'une colonne) en fonction de son nom (Desgraupes, 2013, p10).
...
Un *factor* constitue un objet particulier de R employé" pour stocker les modalités d'un caractères qualitatif. 
Un caractère est dit *qualitatif* lorsque son observation ne peut être traduite par une mesure. Il se décline selon différents états dénombrables appelés *modalités*. Les niveaux du facteur appelés *levels* correspondent à l'ensemble des modalités du caractère.
L'inspection des modalités d'un caractère qualitatif s'effectue à l'aide de la fonction *levels* ou *unique* (R base). 
...
### Les vecteurs numériques (Integer ou numeric)

Objets particuliers de R pour représenter les variables numeriques. 

Un caractères est dit quantitatif si ses modalités sont mesurables. Le caractère quantitatif prend alors le nom de *variable statistique* et ses différentes modalités sont les *valeurs* possibles de la variable (py 1996).
