...
La sélection selon une combinaison de modalités appartenant à un caractère s'effectue par une expression régulière introduite par une fonction d'identification et utilisant le signe | (barre verticale) comme opérateur d'alternation.
...
Dplyr fait appel à la fonction *arrange*  pour trier le contenu des colonnes du talbeau de données. Cette dernière classe les valeurs par ordre croissant et les modalités par ordre alphabétique. Un classement par ordre décroissant s'obtient par l'ajout de l'argument *desc*.
...
Identification des navires caractrisés par un nom et un tonnage similaire.
...
## Ommission des données manquantes

L'omission des données manquantes s'effectue à la l'aide la fonction complete.cases {stats}
```{r eval = FALSE}
