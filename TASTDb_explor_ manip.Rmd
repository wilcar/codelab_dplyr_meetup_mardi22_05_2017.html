...
La fonction *colnames * est utilis?e pour obtenir un affichage s?quentiel des noms de colonnes. Chaque colonne est caract?ris? par un indice qui est sa position dans le tableau. Afin de se rep?rer, R ?crit entre crochet dans la marge de gauche l'indice du premier ?lement de la ligne.  
La fonction wich de R est une fonction primitive de R qui permet de trouver les indices de position d'un vecteur (d'une colonne) en fonction d'un condition ou propri?t? (Desgraupes, 2013, p10)

```{r eval = FALSE}
# Fonction names {base}: Affichage index? de la d?signation des colonnes du tableau 
colnames (voyages)
